...
import (
	...
	"dts/backend/internal/cdc/sqlservercdc"
...
func run(ctx context.Context) error {
	if v := strings.ToLower(env("DTS_EXPERIMENTAL_SQLSERVER_NATIVE", "")); v != "1" && v != "true" && v != "yes" && v != "on" {
		return errors.New("DTS_EXPERIMENTAL_SQLSERVER_NATIVE=1 is required")
	}
	if v := strings.ToLower(env("DTS_EXPERIMENTAL_SQLSERVER_CDC", "")); v != "1" && v != "true" && v != "yes" && v != "on" {
		return errors.New("DTS_EXPERIMENTAL_SQLSERVER_CDC=1 is required")
	}
	...
	host := env("DTS_SQLSERVER_HOST", "")
	user := env("DTS_SQLSERVER_USER", "")
	start := env("DTS_SQLSERVER_START_LSN", "")
	if host == "" || user == "" || start == "" {
		return errors.New("SQLSERVER_HOST, USER and START_LSN are required")
	}
	port, _ := strconv.Atoi(env("DTS_SQLSERVER_PORT", "1433"))
	if port <= 0 {
		port = 1433
	}
	ds := domain.DataSource{Type: domain.DataSourceSQLServer, Host: host, Port: port, Username: user, Password: env("DTS_SQLSERVER_PASSWORD", ""), Database: env("DTS_SQLSERVER_DATABASE", "master"), TLSMode: domain.TLSMode(strings.ToUpper(env("DTS_SQLSERVER_TLS_MODE", "PREFERRED"))), TLSServerName: env("DTS_SQLSERVER_TLS_SERVER_NAME", ""), TLSCACert: os.Getenv("DTS_SQLSERVER_TLS_CA"), TLSClientCert: os.Getenv("DTS_SQLSERVER_TLS_CLIENT_CERT"), TLSClientKey: os.Getenv("DTS_SQLSERVER_TLS_CLIENT_KEY")}
	...
	selected := []string{}
	for _, v := range strings.Split(env("DTS_SQLSERVER_TABLES", ""), ",") {
		if x := strings.TrimSpace(v); x != "" {
			selected = append(selected, x)
		}
	}
	if len(selected) == 0 {
		return errors.New("DTS_SQLSERVER_TABLES is required")
	}
	...
	poll, _ := time.ParseDuration(env("DTS_SQLSERVER_CDC_POLL", "1s"))
	maxTx, _ := strconv.Atoi(env("DTS_SQLSERVER_CDC_MAX_TRANSACTIONS", "256"))
