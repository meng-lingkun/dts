...
import (
	...
	"dts/backend/internal/cdc/oraclelogminer"
...
func run(ctx context.Context) error {
	if !enabled("DTS_EXPERIMENTAL_ORACLE_NATIVE") {
		return errors.New("DTS_EXPERIMENTAL_ORACLE_NATIVE=1 is required")
	}
	if !enabled("DTS_EXPERIMENTAL_ORACLE_LOGMINER_CDC") {
		return errors.New("DTS_EXPERIMENTAL_ORACLE_LOGMINER_CDC=1 is required")
	}
	...
	host, user, start := env("DTS_ORACLE_HOST", ""), env("DTS_ORACLE_USER", ""), env("DTS_ORACLE_START_SCN", "")
	if host == "" || user == "" || start == "" {
		return errors.New("ORACLE_HOST, USER and START_SCN are required")
	}
	port, _ := strconv.Atoi(env("DTS_ORACLE_PORT", "1521"))
	if port <= 0 {
		port = 1521
	}
	ds := domain.DataSource{Type: domain.DataSourceOracle, Host: host, Port: port, Username: user, Password: env("DTS_ORACLE_PASSWORD", ""), Database: env("DTS_ORACLE_SERVICE", ""), TLSMode: domain.TLSMode(strings.ToUpper(env("DTS_ORACLE_TLS_MODE", "DISABLE"))), TLSServerName: env("DTS_ORACLE_TLS_SERVER_NAME", ""), TLSCACert: os.Getenv("DTS_ORACLE_TLS_CA"), TLSClientCert: os.Getenv("DTS_ORACLE_TLS_CLIENT_CERT"), TLSClientKey: os.Getenv("DTS_ORACLE_TLS_CLIENT_KEY")}
	...
	selected := []string{}
	for _, v := range strings.Split(env("DTS_ORACLE_TABLES", ""), ",") {
		if x := strings.TrimSpace(v); x != "" {
			selected = append(selected, x)
		}
	}
	if len(selected) == 0 {
		return errors.New("DTS_ORACLE_TABLES is required")
	}
	...
	poll, _ := time.ParseDuration(env("DTS_ORACLE_CDC_POLL", "2s"))
	span, _ := strconv.ParseUint(env("DTS_ORACLE_CDC_MAX_SCN_SPAN", "100000"), 10, 64)
