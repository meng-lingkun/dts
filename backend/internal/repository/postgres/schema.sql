...
-- Durable, expiring control-plane leases prevent duplicate preparation and
-- validation when more than one API server is running or a server restarts.
...
CREATE INDEX IF NOT EXISTS idx_control_operation_leases_expiry ON control_operation_leases(lease_until);
