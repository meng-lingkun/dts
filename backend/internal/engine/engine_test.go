...
func TestUnifiedFullRender(t *testing.T) {
	...
	if cfg.Engine != "dts" || len(cfg.Command) != 0 {
		t.Fatalf("FULL must stay inside DTS data plane: %+v", cfg)
	}
...
func TestUnifiedMySQLCDCRender(t *testing.T) {
	...
	if cfg.Engine != "dts" || len(cfg.Command) != 1 || cfg.Command[0] != "dts-mysql-cdc" {
		t.Fatalf("unexpected unified MySQL CDC config: %+v", cfg)
	}
	if cfg.Env["DTS_MYSQL_START_FILE"] != "mysql-bin.000123" || cfg.Env["DTS_MYSQL_START_POS"] != "4567" {
		t.Fatalf("bad MySQL start config: %+v", cfg.Env)
	}
	if cfg.Env["DTS_MYSQL_SOURCE_TYPE"] != string(domain.DataSourcePolarDBMySQL) || cfg.Env["DTS_MYSQL_TLS_MODE"] != "REQUIRED" || cfg.Env["DTS_MYSQL_TLS_SERVER_NAME"] != "mysql.internal" || cfg.Env["DTS_MYSQL_TLS_CA"] != "ca-pem" || cfg.Env["DTS_MYSQL_TLS_CLIENT_CERT"] != "cert-pem" || cfg.Env["DTS_MYSQL_TLS_CLIENT_KEY"] != "key-pem" {
...
func TestUnifiedOceanBaseBinlogCDCRender(t *testing.T) {
	...
	if cfg.Env["DTS_MYSQL_HOST"] != "odp-binlog" || cfg.Env["DTS_MYSQL_PORT"] != "2883" {
		t.Fatalf("OceanBase CDC must use cdc_url ODP endpoint: %+v", cfg.Env)
	}
	if cfg.Env["DTS_MYSQL_START_GTID"] != task.CDCStartPositionValue || cfg.Env["DTS_MYSQL_SOURCE_TYPE"] != string(domain.DataSourceOceanBase) {
		t.Fatalf("bad OceanBase GTID/source config: %+v", cfg.Env)
	}
	if cfg.Env["DTS_MYSQL_TLS_MODE"] != "REQUIRED" || cfg.Env["DTS_MYSQL_TLS_SERVER_NAME"] != "odp.example" {
		t.Fatalf("obbinlogs must enforce CDC endpoint TLS: %+v", cfg.Env)
	}
	if cfg.Env["DTS_MYSQL_FAILOVER_ENDPOINTS"] != "odp2:2883" {
		t.Fatalf("OceanBase CDC failover endpoints not propagated: %+v", cfg.Env)
	}
}

func TestUnifiedPostgresCDCRender(t *testing.T) {
	task := &domain.MigrationTask{ID: "pg-task", Mode: domain.ModeIncremental, CDCStartPositionType: "LSN", CDCStartPositionValue: "0/16B6C50", CDCStartResource: "dts_pg_task"}
	...
	if cfg.Engine != "dts" || len(cfg.Command) != 1 || cfg.Command[0] != "dts-postgres-cdc" || cfg.Env["DTS_PG_SLOT"] != "dts_pg_task" {
...
func TestUnifiedOpenGaussCDCRender(t *testing.T) {
	...
	task := &domain.MigrationTask{ID: "og", Mode: domain.ModeIncremental, CDCStartPositionType: "OPENGAUSS_LSN", CDCStartPositionValue: "0/16B6C50", CDCStartResource: "dts_og"}
	...
	if cfg.Engine != "dts" || len(cfg.Command) != 1 || cfg.Command[0] != "dts-opengauss-cdc" {
		t.Fatalf("bad openGauss CDC config: %+v", cfg)
	}
	if cfg.Env["DTS_OPENGAUSS_SLOT"] != "dts_og" || cfg.Env["DTS_OPENGAUSS_START_LSN"] != "0/16B6C50" || cfg.Env["DTS_OPENGAUSS_TLS_MODE"] != "REQUIRED" || cfg.Env["DTS_OPENGAUSS_TLS_SERVER_NAME"] != "og.internal" {
		t.Fatalf("bad openGauss CDC env: %+v", cfg.Env)
	}
	if cfg.Env["DTS_EXPERIMENTAL_OPENGAUSS_LOGICAL_CDC"] != "1" {
...
func TestUnifiedKingbaseCDCRender(t *testing.T) {
	...
	task := &domain.MigrationTask{ID: "kb-task", Mode: domain.ModeIncremental, CDCStartPositionType: "KINGBASE_LSN", CDCStartPositionValue: "0/16B6C50", CDCStartResource: "dts_kb"}
	...
	if cfg.Engine != "dts" || len(cfg.Command) != 1 || cfg.Command[0] != "dts-postgres-cdc" {
		t.Fatalf("bad Kingbase CDC config: %+v", cfg)
	}
	if cfg.Env["DTS_PG_SOURCE_TYPE"] != string(domain.DataSourceKingbase) || cfg.Env["DTS_PG_SLOT"] != "dts_kb" || cfg.Env["DTS_PG_START_LSN"] != "0/16B6C50" {
		t.Fatalf("bad Kingbase CDC env: %+v", cfg.Env)
	}
	if cfg.Env["DTS_EXPERIMENTAL_KINGBASE_LOGICAL_CDC"] != "1" || cfg.Env["DTS_PG_TLS_MODE"] != "REQUIRED" || cfg.Env["DTS_PG_TLS_SERVER_NAME"] != "kb.internal" {
		t.Fatalf("Kingbase gate/TLS config missing: %+v", cfg.Env)
	}
	if !strings.Contains(cfg.Env["DTS_PG_PUBLICATION"], "dts_pub_kb_task") {
		t.Fatalf("unexpected Kingbase publication: %+v", cfg.Env)
	}
}

func TestUnifiedGaussDBCDCRender(t *testing.T) {
	task := &domain.MigrationTask{ID: "gauss", Mode: domain.ModeIncremental, CDCStartPositionType: "GAUSSDB_LSN", CDCStartPositionValue: "0/16B6C50", CDCStartResource: "dts_gauss"}
	...
	if cfg.Engine != "dts" || len(cfg.Command) != 1 || cfg.Command[0] != "dts-gaussdb-cdc" {
		t.Fatalf("bad GaussDB CDC config: %+v", cfg)
	}
	if cfg.Env["DTS_GAUSSDB_SLOT"] != "dts_gauss" || cfg.Env["DTS_GAUSSDB_START_LSN"] != "0/16B6C50" || cfg.Env["DTS_GAUSSDB_TLS_MODE"] != "REQUIRED" || cfg.Env["DTS_GAUSSDB_TLS_SERVER_NAME"] != "gauss.internal" {
		t.Fatalf("bad GaussDB CDC env: %+v", cfg.Env)
	}
	if cfg.Env["DTS_EXPERIMENTAL_GAUSSDB_NATIVE"] != "1" || cfg.Env["DTS_EXPERIMENTAL_GAUSSDB_LOGICAL_CDC"] != "1" {
...
func TestUnifiedGaussDBDDLRenderRequiresExplicitSourcePolicy(t *testing.T) {
	task := &domain.MigrationTask{ID: "gauss-ddl", Mode: domain.ModeIncremental, CDCStartPositionType: "GAUSSDB_LSN", CDCStartPositionValue: "0/16B6C50", CDCStartResource: "dts_gauss_ddl", CDCDDLMode: "SAME_FAMILY"}
	...
	if cfg.Env["DTS_GAUSSDB_DDL_REPLAY"] != "1" {
		t.Fatalf("GaussDB SAME_FAMILY DDL did not enable reader DDL mode: %+v", cfg.Env)
	}
	if _, ok := cfg.Env["DTS_GAUSSDB_DDL_ONLY_TRANSACTIONS"]; ok {
...
func TestUnifiedSQLServerCDCRender(t *testing.T) {
	...
	if cfg.Engine != "dts" || len(cfg.Command) != 1 || cfg.Command[0] != "dts-sqlserver-cdc" {
		t.Fatalf("bad SQL Server CDC config: %+v", cfg)
	}
	if cfg.Env["DTS_SQLSERVER_START_LSN"] != task.CDCStartPositionValue || cfg.Env["DTS_SQLSERVER_TLS_MODE"] != "REQUIRED" {
...
func TestUnifiedOracleCDCRender(t *testing.T) {
	...
	src := domain.DataSource{Type: domain.DataSourceOracle, Host: "ora", Port: 1521, Username: "dts", Password: "secret", Database: "ORCLPDB1"}
	dst := domain.DataSource{Type: domain.DataSourcePolarDBX, Host: "dst", Port: 8527}
	tables := []domain.MigrationTable{{SourceSchema: "APP", SourceTable: "ORDERS"}}
	cfg, err := NewUnified().Render(context.Background(), task, src, dst, tables)
	if err != nil {
	...
	if cfg.Engine != "dts" || len(cfg.Command) != 1 || cfg.Command[0] != "dts-oracle-cdc" {
		t.Fatalf("bad Oracle CDC config: %+v", cfg)
	}
	if cfg.Env["DTS_ORACLE_START_SCN"] != "123456" || cfg.Env["DTS_EXPERIMENTAL_ORACLE_LOGMINER_CDC"] != "1" {
...
func TestUnifiedDB2CDCRender(t *testing.T) {
	...
	if cfg.Engine != "dts" || len(cfg.Command) != 1 || cfg.Command[0] != "dts-db2-cdc" {
		t.Fatalf("bad DB2 CDC config: %+v", cfg)
	}
	if cfg.Env["DTS_DB2_START_LRI"] != task.CDCStartPositionValue || cfg.Env["DTS_EXPERIMENTAL_DB2_LOG_CDC"] != "1" {
...
func TestUnifiedDamengCDCRender(t *testing.T) {
	...
	if cfg.Engine != "dts" || len(cfg.Command) != 1 || cfg.Command[0] != "dts-dameng-cdc" {
		t.Fatalf("bad Dameng CDC config: %+v", cfg)
	}
	if cfg.Env["DTS_DAMENG_START_LSN"] != "12345" || cfg.Env["DTS_EXPERIMENTAL_DAMENG_LOG_CDC"] != "1" {
...
func TestUnifiedGBase8aCDCRender(t *testing.T) {
	...
	if len(cfg.Command) != 1 || cfg.Command[0] != "dts-gbase-cdc" || cfg.Env["DTS_GBASE8A_CDC_START_POSITION"] != task.CDCStartPositionValue {
		t.Fatalf("cfg=%+v", cfg)
	}
	if !strings.Contains(cfg.Env["DTS_GBASE8A_CDC_SELECTIONS_JSON"], `"schema_fingerprint"`) {
		t.Fatalf("selection=%s", cfg.Env["DTS_GBASE8A_CDC_SELECTIONS_JSON"])
	}
}
...
func TestRegistryExposesSingleUnifiedEngine(t *testing.T) {
	...
	if len(infos) != 1 || infos[0].Name != "dts" || !infos[0].Available {
		t.Fatalf("unexpected engine registry: %+v", infos)
	}
}

func TestUnifiedGaussDBDDLRenderRejectsNonIdentityTarget(t *testing.T) {
	task := &domain.MigrationTask{ID: "gauss-ddl-bad", Mode: domain.ModeIncremental, CDCStartPositionType: "GAUSSDB_LSN", CDCStartPositionValue: "0/16B6C50", CDCStartResource: "dts_gauss_ddl", CDCDDLMode: "SAME_FAMILY"}
	src := domain.DataSource{Type: domain.DataSourceGaussDB, Host: "gauss", Port: 8000, Username: "repl", Password: "secret", Database: "app"}
...
func TestUnifiedGBase8sCDCRender(t *testing.T) {
	...
	if len(cfg.Command) != 1 || cfg.Command[0] != "dts-gbase8s-cdc" || cfg.Env["DTS_GBASE8S_CDC_START_POSITION"] != "restart=50;commit=100" {
		t.Fatalf("cfg=%+v", cfg)
	}
	selJSON := cfg.Env["DTS_GBASE8S_CDC_SELECTIONS_JSON"]
