...
DTS RC28 supports two datasource-local provider mechanisms for the
`syscdcv1` smart-LOB CDC stream:

1. **Native C ABI v4 (recommended)** — build a normal Linux `.so` with the local
   GBase 8s Client-SDK / ESQL-C toolchain. The DTS agent loads it with
   `dlopen`, so the provider does not need the same Go toolchain as DTS.
2. Legacy Go `buildmode=plugin` — retained for RC20 compatibility only.

The vendor CSDK itself is not shipped by DTS.

## Native C ABI

Implement the symbols in:

- `dts_gbase8s_cdc_provider.h`
...
- lossless conversion of CDC row values into the DTS `CDCField` JSON
  contract (`base64` for arbitrary bytes).

DTS still owns transaction assembly, long-transaction restart
...
export DTS_GBASE8S_CDC_PROVIDER_SHA256='<64-hex-sha256>'
...
export DTS_GBASE8S_CDC_AGENT_TOKEN='...'
./bin/dts-gbase8s-cdc-agent
```

`DTS_GBASE8S_CDC_PROVIDER_LIBRARY` must be an absolute, regular,
...
```bash
export GBASE8S_CDC_PROVIDER_DIR=/opt/src/dts-gbase8s-csdk-provider
./deployments/scripts/build-gbase8s-cdc-provider-plugin.sh
export DTS_GBASE8S_CDC_PROVIDER_PLUGIN=$PWD/bin/dts-gbase8s-cdc-provider.so
```
