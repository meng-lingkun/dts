...
DTS V0.15.0-rc25 retains the qualification-gated Dameng data plane introduced in RC13 and adds an experimental archived-log source CDC path plus exact `DM_LSN` validation snapshots. DTS owns metadata discovery, Full Load planning/read/write, schema creation, prepared target DML, point lookup and transactional CDC target apply. SQL transport is provided by DM's Go `database/sql` driver; DTS does not vendor the proprietary DM wire driver in its source archive.
...
- `dts-dameng-qualify` structured real-instance qualification.
...
- same-`COMMIT_SCN` XIDs aggregated into one DTS target transaction so one durable `DM_LSN` cannot suppress a sibling transaction;
...
DTS intentionally keeps those vendor sources outside this archive. On Linux, the stock RC13 Server/Worker can load a Go plugin whose `init` registers the DM driver:

```bash
export DM_GO_DRIVER_DIR=/opt/dmdbms/drivers/go/dm
./deployments/scripts/build-dameng-driver-plugin.sh

export DTS_DAMENG_DRIVER_PLUGIN="$PWD/bin/dts-dameng-driver.so"
export DTS_EXPERIMENTAL_DAMENG_NATIVE=1
```

The plugin must be built with the same Go toolchain as the DTS binaries. If a custom provider registers another `database/sql` driver name, set `DTS_DAMENG_SQL_DRIVER` or datasource `driver_class` accordingly.
...
export DAMENG_QUALIFY_OUTPUT=/tmp/dts-dameng-qualification.json
./deployments/scripts/qualify-dameng.sh
```
...
The CDC check requires `ARCH_INI=1`, logical redo (`RLOG_APPEND_LOGIC`), `ENABLE_FLASHBACK=1`, uncompressed logical redo for the RC25 software contract, a deterministic primary key, and continuous local archived-log coverage. DTS uses archived `DBMS_LOGMNR` records only as the XID/ROWID/commit index and reconstructs row images with flashback `AS OF SCN`; unsupported DDL/BATCH_UPDATE/unknown operations or an archive/UNDO gap fail closed before checkpoint advancement.

The report never includes the password. Keep the JSON report together with the exact DM release, Go driver release, server charset, deployment mode and DTS build digest.
