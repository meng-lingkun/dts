# DTS RC29 Predictive Performance Control
...
Defaults derive from `DTS_CDC_SPOOL_MAX_PENDING_BYTES` (64 GiB): WARN at 50%, CRITICAL at 80%. Operators may override:

```bash
DTS_CDC_SPOOL_BACKLOG_WARN_BYTES
DTS_CDC_SPOOL_BACKLOG_CRITICAL_BYTES
DTS_CDC_SPOOL_PREDICT_WARN_SECONDS=900
DTS_CDC_SPOOL_PREDICT_CRITICAL_SECONDS=300
```
...
```bash
DTS_ADAPTIVE_BATCH_TARGET_MS=1200
DTS_ADAPTIVE_BATCH_MIN_ROWS=50
DTS_ADAPTIVE_BATCH_MAX_ROWS=5000
```
...
```text
dts_cdc_spool_growth_bytes_per_second
dts_cdc_spool_critical_eta_seconds
dts_task_effective_parallelism
dts_cdc_spool_pending_bytes
dts_cdc_spool_storage_used_pct
```
