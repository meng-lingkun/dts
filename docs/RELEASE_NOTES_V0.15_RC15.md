# DTS V0.15.0-rc15 Release Notes
...
- DTS decodes the documented big-endian B/C/I/U/D frame protocol rather
...
- PostgreSQL-compatible `bytea` OID 17 values are decoded from their `\\x...`
  representation into byte-preserving DTS CDC fields.
...
- DTS validates BEGIN/COMMIT/XID/LSN relationships and complete
  transaction boundaries before returning a transaction to the runtime.
- Target transaction commit and durable DTS checkpoint still occur
...
`dts-gaussdb-qualify --cdc` now creates a temporary LSN-based
...
GaussDB exposes documented DDL decoding, but DTS's current target apply
