...
COM_BINLOG_DUMP. DTS now has a dedicated TiCDC control/data adapter while
...
- DTS creates/reuses a deterministic changefeed per task/direction.
...
- Added a DTS-owned pure-Go Kafka metadata/fetch consumer. No kcat,
...
- Consecutive DML messages with the same TiDB `commitTs` are assembled into one
  DTS target transaction before apply.
...
- Source ACK still occurs only after target apply + durable DTS checkpoint.
...
deletes the TiCDC changefeed. DTS does not delete the external Kafka topic.
...
- RC3 intentionally uses one partition. Multi-partition transaction merge is not
  claimed.
- TiCDC itself is the TiDB-native upstream capture component; DTS owns the
