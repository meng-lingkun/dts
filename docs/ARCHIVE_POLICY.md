...
1. `dts-<version>.zip`：完整源码快照，不包含 `.git/`、`bin/`、`data/`、`node_modules/`、`dist/`。
2. `dts-<previous>-to-<version>.patch`：上一开发版本到当前版本的增量 Binary Patch。
3. `dts-<version>.patch`：正式 `V0.13` 基线到当前版本的累计 Binary Patch，用于灾难恢复。
4. `dts-<version>.sha256`：归档文件 SHA-256。
5. `dts-<version>.manifest.json`：版本、上一版本、文件大小、SHA-256 和恢复验证结果。
