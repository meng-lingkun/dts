# DTS V0.15.0-unified-dev12 Release Notes

## Scope

This snapshot continues the single DTS Unified Engine and advances the native Oracle connector one layer above dev11 authentication: a bounded, experimental TTC SELECT request/describe/row wire path. It does not add an external migration runtime and it does not enable production Oracle metadata, Full Load or CDC capabilities.

## Experimental Oracle TTC SELECT

- Added a DTS-owned bind-free OALL8 parse+execute request codec for authenticated TTC sessions.
- Added `DTS_EXPERIMENTAL_ORACLE_TTC_QUERY=1`. The query gate implies protocol/datatype negotiation and password authentication, then executes `SELECT 1 AS DTS_PROBE FROM DUAL`.
...
- Oracle DATE is decoded as a timezone-less timestamp string; DTS does not invent a timezone the source value does not carry.
