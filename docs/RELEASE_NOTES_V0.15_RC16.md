# DTS V0.15.0-rc16 Release Notes
...
- DML values are never taken from that text pass. DTS peeks the binary
...
- Target apply and durable DTS checkpoint still happen before either
...
stream. DTS therefore cannot reconstruct such a transaction from the
...
- worker environment `DTS_GAUSSDB_DDL_ONLY_TRANSACTIONS=1`;
