...
DTS 的目标不是管理 DataX、SeaTunnel、Flink CDC、Debezium、Canal，而是把这些项目中成熟的工程思想重新实现为一个统一的 DTS Runtime。
...
从 V0.15 起，任务中的历史 `full_engine/cdc_engine/rollback_cdc_engine` 字段只用于兼容旧元数据，服务端统一归一化为 `dts`。Web 不再提供引擎选择页。
