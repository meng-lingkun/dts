# QMigration V0.15.0-rc11 Support Matrix
...
vendor API rather than a SQL/wire log API; RC11 uses this model for Db2
...
| **DB2 LUW** | **Yes** | **Yes, EXTDTA LOB + VECTOR_SERIALIZE source** | **Yes, Prepared SQLDTA/EXTDTA; VECTOR target rejected** | **QMigration Log Agent + IBM db2ReadLog / DB2_LRI** | **Yes** | **EXPERIMENTAL; qualification required** |
| Dameng / DM | No | No | No | No | No | PROBE_ONLY |
| GaussDB | No | No | No | No | No | PROBE_ONLY |
| GBase | No | No | No | No | No | PROBE_ONLY |

## DB2 RC11 source CDC scope
...
- explicit fail-closed DB2 VECTOR target auto-create/prepared write;
...
Fail-closed boundaries in RC11:
...
- DB2 VECTOR target schema/write until dimension/coordinate metadata and native target parameter encoding are implemented;
...
2. DB2 real-instance out-of-row multi-insert/VECTOR source qualification, VECTOR target support, pureScale qualification and retained decomposed-update rollback traces.
