# DTS V0.15.0-rc26 Release Notes
...
- `DTS_EXPERIMENTAL_OPENGAUSS_LOGICAL_CDC=1` exposes source CDC only
...
- Target transaction + DTS durable checkpoint complete before
...
- Adds `dts-opengauss-cdc` and `dts-opengauss-qualify`.

## KingbaseES source CDC

- `DTS_EXPERIMENTAL_KINGBASE_LOGICAL_CDC=1` exposes source CDC only
  after explicit qualification.
...
- Before opening a stream, DTS proves that the durable slot still uses
...
- Adds `dts-kingbase-qualify`; the existing PostgreSQL CDC worker is
