# QMigration V0.15.0-rc21 Support Matrix
...
| **GBase 8s V8.8** | **Yes** | **Yes** | **Full Write + transactional target CDC Apply** | **syscdcv1/CSDK smart-LOB via native C ABI v1 provider; GBASE8S_CDC_SEQ** | **Target owner check + table/PK/index/FK apply** | **EXPERIMENTAL / qualification required** |
...
- RC21 preferred source provider is a Linux native C ABI v1 `.so` loaded by
...
1. Build the native C ABI provider against a real GBase 8s V8.8 Client-SDK and retain long-transaction/restart/failover qualification.
2. Qualify source TRUNCATE and supported smart BLOB/CLOB retrieval separately before widening CDC type support.
